...
typedef struct
{
	...
	bool    whodunnit;    /* whodunnit flag for each transaction */
...
tuple_to_stringinfo(LogicalDecodingContext *ctx, TupleDesc tupdesc, HeapTuple tuple, TupleDesc indexdesc, bool replident, bool hasreplident)
{
	...
	if (replident)
	{
		if (data->pretty_print)
		{
			...
			appendStringInfoString(&keyvaues, "\t\t\t\"change_sets\":{\n");
		}
		else
		{
			/*appendStringInfoString(&colnames, "\"oldkeys\":{");
	...
	else
	{
		if (data->pretty_print)
		{
			...
			appendStringInfoString(&keyvalues, "\t\t\t\"object\": {\n");
		}
		else
		{
			/*appendStringInfoString(&colnames, "\"columnnames\":[");
	...
	for (natt = 0; natt < tupdesc->natts; natt++)
	{
		...
		/* write column name */
		appendStringInfo(&keyvalues, "\"%s\":", NameStr(attr->attname));
		...
		if (isnull)
		{
		  ...
		  appendStringInfo(&keyvalues, "null%s", comma);
		}
		else
		{
			if (typisvarlena)
			...
			switch (typid)
			{
				...
				case NUMERICOID:
							...
							pg_strncasecmp(outputstr, "-Infinity", 9) == 0)
					{
						...
						appendStringInfo(&keyvalues, "%s%s", outputstr, comma);
					else
				...
				case BOOLOID:
					if (strcmp(outputstr, "t") == 0)
						...
						appendStringInfo(&keyvalues, "true%s", comma);
					else
						appendStringInfo(&colvalues, "false%s", comma);
					break;
				default:
					appendStringInfoString(&colvalues, comma);
					quote_escape_json(&colvalues, outputstr);
					quote_escape_json(&keyvalues, outputstr);
					appendStringInfoString(&keyvalues, comma);
					break;
			}
		}
	...
	if (replident)
	{
		if (data->pretty_print)
		{
			...
			appendStringInfoString(&colvalues, "\t\t\t}\n");
			appendStringInfoString(&keyvalues, "}\n");
		}
		else
		{
			appendStringInfoString(&colnames, "],");
	...
	appendStringInfoSTring(ctx->out, keyvalues.data);

	pfree(colnames.data);
	pfree(coltypes.data);
	pfree(colvalues.data);
	pfree(keyvalues.data);
}
				 ...
				 Relation relation, ReorderBufferChange *change)
{
	...
	switch (change->action)
	{
		case REORDER_BUFFER_CHANGE_INSERT:
			...
			/**** print whodunnit insert event different ****/
			if (strcmp(NameStr(class_form->relname), "whoswho") == 0 )
			{
	...
	switch (change->action)
	{
		case REORDER_BUFFER_CHANGE_INSERT:
			if (data->pretty_print)
				appendStringInfoString(ctx->out, "\t\t\t\"event\": \"insert\",\n");
			else
				appendStringInfoString(ctx->out, "\"event\":\"insert\",");
			break;
		case REORDER_BUFFER_CHANGE_UPDATE:
			if(data->pretty_print)
				appendStringInfoString(ctx->out, "\t\t\t\"event\": \"update\",\n");
			else
				appendStringInfoString(ctx->out, "\"event\":\"update\",");
			break;
		case REORDER_BUFFER_CHANGE_DELETE:
			if (data->pretty_print)
				appendStringInfoString(ctx->out, "\t\t\t\"event\": \"delete\",\n");
			else
				appendStringInfoString(ctx->out, "\"event\":\"delete\",");
			break;
		default:
